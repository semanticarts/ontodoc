...
class Create(Command):
    "The basic command to initialize an empty ontodoc instance"
    ...
    def take_action(self, parsed_args):
        if is_created():
            self.app.stdout.write('Error: It appears there is already '
                                  'an ontodoc site in this directory.\n'
                                  'To create a new site, delete '
                                  '_config.yaml and rerun `ontodoc new`\n')
        else:
            self.app.stdout.write('Creating ontodoc site...\n')
            curr_dir = os.getcwd()
...
def is_created():
    ...
    """ Checks to see if `ontodoc new` command has already been run on dir """
